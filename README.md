...
- 배치 스크립트에는 테스트 스크립트 명령어만 입력 할 수 있습니다.
...
아래는 테스트 스크립트를 실행 할 수 있는 셸 명령어입니다.
- `1_FullWriteAndReadCompare`
- `2_PartialLBAWrite`
- `3_WriteReadAging`
- `4_EraseAndWriteAgin`
...
1. **1_FullWriteAndReadCompare**: 모든 LBA에 순차적으로 데이터를 쓰고 읽어서 검증합니다.
2. **2_PartialLBAWrite**: 특정 LBA 세트(4,0,3,1,2)에 반복적으로 데이터를 쓰고 검증합니다.
3. **3_WriteReadAging**: 특정 LBA(0,99)에 200회 반복 쓰기/읽기 테스트를 수행합니다.
4. **4_EraseAndWriteAgin**: erase 명령 후 쓰기 및 읽기 테스트를 수행합니다.
